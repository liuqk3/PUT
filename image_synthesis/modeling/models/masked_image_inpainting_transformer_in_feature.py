...
class FullAttention(nn.Module):
    ...
    def count_flops(m, x, y):
        ...
        B, T, C = x[0].shape
        # qkv linear
        total_ops += m.key.total_ops
        total_ops += m.query.total_ops
        total_ops += m.value.total_ops

        # atten
        head_dim = m.key.out_features // m.n_head
        total_ops += (B*m.n_head*T*T*head_dim) # bmm, matrix multiply
        total_ops += (B*m.n_head*T*T) # x scale
        total_ops += calc_func.calculate_softmax(B*m.n_head*T, T) # softmax
        total_ops += (B*m.n_head*T*C*T)# bmm, matrix multiply
...
class MaskedImageInpaintingTransformer(nn.Module):
    ...
    def sample_largest(
        ...
        **kwargs,
        ...
        if mask_low_to_high:
            ...
            # import pdb; pdb.set_trace()
            im_mask = F.interpolate(batch['mask'].float(), size=low_res, mode='nearest')
            im_mask = F.interpolate(im_mask, size=ori_res, mode='nearest').bool()            
        else:
            im_mask = batch['mask']
        ...
        token_type, _ = get_token_type(im_mask, type='pixel_shuffle', token_shape=[h,w]) # B x 1 x H x W
    ...
    def sample( # sample_raster
        ...
        **kwargs,
        ...
        token_type, _ = get_token_type(mask_, type='pixel_shuffle', token_shape=[h,w]) # B x 1 x H x W
    ...
    def prepare_data(self, image, mask):
        ...
        if self.input_feature_type == 'origin':
            ...
            token_type, _ = get_token_type(mask, type='pixel_shuffle', token_shape=[h,w]) # B x 1 x H x W
