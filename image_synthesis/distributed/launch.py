...
def launch(fn, n_gpu_per_machine, n_machine=1, machine_rank=0, dist_url=None, backend=None, args=()):
    if backend is None:
        backend = 'nccl'
    ...
    if world_size > 1:
        ...
        mp.spawn(
            ...
            args=(backend, fn, world_size, n_gpu_per_machine, machine_rank, dist_url, args),
            daemon=False,
        )
...
def distributed_worker(
    local_rank, backend, fn, world_size, n_gpu_per_machine, machine_rank, dist_url, args
):
    ...
    try:
        ...
        raise OSError("failed to initialize {} groups".format(backend))
