...
class MaskedImageInpaintingTransformerSegConcatUnknownClassSketch(nn.Module):
    def __init__(
        self,
        *,
        ...
        sketch_codec_config=None,
        seg_codec_config=None,
        ...
        self.sketch_codec = instantiate_from_config(sketch_codec_config)
        self.seg_codec = instantiate_from_config(seg_codec_config)

        self.prob_sample = prob_sample
        
        self.sketch_embed_default = nn.Embedding(1, dim//content_patch_seq_len//3)
        self.seg_embed_default = nn.Embedding(1, dim//content_patch_seq_len//3)

        feat_emb_dim = dim//content_patch_seq_len - (dim//content_patch_seq_len//3)*2
        # self.emb_proj = nn.Linear(self.content_codec.embed_dim, feat_emb_dim)
        # self.sketch_emb_proj = nn.Linear(self.sketch_codec.embed_dim, dim//content_patch_seq_len//3)

        self.pos_emb = nn.Parameter(torch.zeros(1, content_seq_len, feat_emb_dim))
        if learn_mask_emb:
            self.mask_emb = nn.Parameter(torch.zeros(1, feat_emb_dim, 1, 1))
        else:
            self.mask_emb = None
        ...
        seg_config = setup_cfg(seg_config_file)
        self.seg_model_params = {'config_file': seg_config, 'model_weight': seg_model_weight}
        self.content_codec = instantiate_from_config(content_codec_config)
        self.sketch_codec = instantiate_from_config(sketch_codec_config)
        self.seg_codec = instantiate_from_config(seg_codec_config)
        ...
        self.unknow_class_param = unknow_class_param
        # num_embeddings = seg_config.MODEL.POSITION_HEAD.THING.NUM_CLASSES + seg_config.MODEL.POSITION_HEAD.STUFF.NUM_CLASSES + unknow_class_param['num_unknow_classes']
        # num_embeddings = seg_config.MODEL.SEM_SEG_HEAD.NUM_CLASSES + unknow_class_param['num_unknow_classes']
        # self.semantic_embedding = nn.Embedding(num_embeddings=num_embeddings, embedding_dim=dim//content_patch_seq_len//3)

        # self.seg_model = Mask2Former(self.seg_model_params['config_file'], self.seg_model_params['model_weight'])
        # self.seg_model.eval()

        self.num_unknown_classes = self.unknow_class_param['num_unknow_classes']

        # self.integrate_sketch = integrate_sketch
        self.sketch_param = sketch_param
    ...
    def init_seg_model(self):
        self.seg_model = Mask2Former(self.seg_model_params['config_file'], self.seg_model_params['model_weight']).to(self.device)
        self.seg_model.eval()


    def init_sketch_model(self):
        self.sketch_model = DexiNed().to(self.device)
        self.sketch_model.load_state_dict(torch.load(self.sketch_param['ckpt_path'], map_location=self.device))
        self.sketch_model.eval()
        for p, np in self.sketch_model.named_parameters():
    ...
    def generate_content(
        ...
        **kwargs,
        ...
        # if self.integrate_sketch and self.sketch_model is None:
        #     self.init_sketch_model()
    ...
    def sample(
        ...
        **kwargs,
        ...
        if 'seg_result' not in batch.keys():
            # seg_result = self.seg_model.getTranSizeResult(batch['image'], mask_, size=self.content_codec.token_shape)
            seg_result = self.seg_model(batch['image'], mask=None, size=None) # B x H x W
            seg_result = seg_result.unsqueeze(dim=1) # B x 1 x H x W
        else:
            seg_result = batch['seg_result']
            # seg_result = batch['seg_result'].long()
            # seg_result = F.interpolate(seg_result.unsqueeze(1).float(), size=self.content_codec.token_shape, mode='nearest').squeeze(1).long()
        if 'sketch_map' in batch.keys():
            sketch_input = batch['sketch_map']
            # print('sketch: 255={}, 0={}'.format((sketch_input[0]==255).sum(), (sketch_input[0]==0).sum()))
        else:
            self.sketch_model.eval()
            sketch_input = self.sketch_model(batch['image'][:,[2,1,0],:,:])

        # sketch_input = self.sketch_model(batch['image'][:,[2,1,0],:,:])

        if mode == 0:
            # no guidance
            seg_emb = self.seg_embed_default(torch.zeros((seg_result.shape[0], self.content_codec.token_shape[0], self.content_codec.token_shape[1])).int().to(seg_result.device))
            sketch_emb = self.sketch_embed_default(torch.zeros((seg_result.shape[0], self.content_codec.token_shape[0], self.content_codec.token_shape[1])).int().to(seg_result.device))
        elif mode == 1:
            # only seg guidance
            # print('seg shape: ', seg_result.shape)
            seg_emb = self.seg_codec.get_features(
                seg_result, 
                mask=None, 
                return_quantize_feature=False,
                return_token=False,
                mask_pixel_value=None)['feature'].permute(0,2,3,1)
            # seg_emb = self.semantic_embedding(seg_result)
            sketch_emb = self.sketch_embed_default(torch.zeros((seg_result.shape[0], self.content_codec.token_shape[0], self.content_codec.token_shape[1])).int().to(seg_result.device))
        elif mode == 2:
            #  only sketch guidance
            seg_emb = self.seg_embed_default(torch.zeros((seg_result.shape[0], self.content_codec.token_shape[0], self.content_codec.token_shape[1])).int().to(seg_result.device))

            sketch_emb = self.sketch_codec.get_features(
                sketch_input, 
                mask=None, 
                return_quantize_feature=False,
                return_token=False,
                mask_pixel_value=None)['feature'].permute(0,2,3,1)
            # sketch_emb = self.sketch_emb_proj(sketch_emb)
        else:
            # both seg and sketch
            seg_emb = self.seg_codec.get_features(
                seg_result, 
        ...
        # seg_attn = self.SegAttn(seg_mask)
        # feat_mask = feat_mask + seg_mask

        # save features before feeding into transformer block
        if False:
            feat_save_path = os.path.join('RESULT/debug/feature_mask_embed_4/feature.pt')
            token_type_save_path = os.path.join(os.path.dirname(feat_save_path), 'token_type.pt')
            os.makedirs(os.path.dirname(feat_save_path), exist_ok=True)
            torch.save(feat_mask.to('cpu'), feat_save_path)
            torch.save(token_type, token_type_save_path)
        ...
        # while content_mask.sum() < content_mask.numel():
            ...
            emb = torch.cat([emb, seg_emb, sketch_emb], dim=-1)
            # seg_emb = pixel_unshuffle(seg_embedding, out_size=content_shape, chunked=True)
            # seg_emb = seg_emb.permute(0, 2, 3, 1)
            # seg_attn = self.SegAttn(seg_embedding)

            assert self.drop is None
            assert not self.attn_content_with_mask
    ...
    def prepare_data(self, image, mask, sketch=None):
        """
    ...
    def seg2unknown(self, seg_map, mask):

        unknown_map = torch.zeros_like(seg_map)

        cnt = 0
        replace_cnt = np.random.randint(low=1, high=self.unknow_class_param['max_number'])

        # probability to add unknown token
        p = np.random.uniform(low=0, high=1)

        num_unknow_class = self.unknow_class_param['num_unknow_classes']
        unknown_class_start = self.unknow_class_param['unknown_class_start']

        if p < self.unknow_class_param['prob_unknow']:

            # mask_resize = F.interpolate(mask.float(), size=seg_map.shape[1:], mode='nearest')
            mask_resize = mask.int()
            mask_ = mask_resize.reshape((mask_resize.shape[0], -1))
  
            while cnt < replace_cnt:

                unknow_class = torch.randint(low=unknown_class_start, high=num_unknow_class+unknown_class_start, size=(seg_map.shape[0], 1))
                unknow_class = unknow_class.to(seg_map.device)

                cnt += 1

                # replace the whole instance
                seg_map_reshape = seg_map.reshape(seg_map.shape[0], -1)
                label = []
                for b in range(seg_map.shape[0]):
                    # print('seg shape:', seg_map_reshape.shape, 'mask shape:', mask_.shape)
                    lb = torch.unique((seg_map_reshape[b]+1)*(1-mask_[b]))
                    idx = lb.nonzero()
                    if len(idx) == 0:
                        label.append(-1)
                        continue
                    lb = (lb[idx] - 1).reshape((-1))
                    label.append(lb)
                    # assert lb.shape == torch.unique((seg_map_reshape[b])).shape

                # label = [torch.unique(seg_map.reshape(seg_map.shape[0], -1)[b]) for b in range(seg_map.shape[0])]
                label_known = []

                # incase to replace the existed unknown token
                for l in label:
                    if not isinstance(l, torch.Tensor):
                        label_known.append(-1)
                        continue
                    l = l.cpu().numpy()
                    l[l>=unknown_class_start] = -1
                    idx = np.where(l==-1)[0]
                    label_known.append(np.delete(l, idx))
                replace_id = [np.random.choice(lk) if isinstance(lk, np.ndarray) and len(lk)!=0 else -1 for lk in label_known]
                replace_id = torch.tensor(replace_id).reshape(-1, 1).to(seg_map.device)
                

                # not replace the already existed unknown token
                tmp = (seg_map == replace_id.unsqueeze(-1).unsqueeze(-1)).to(seg_map.dtype)
                tmp = tmp * (1 - unknown_map)
                tmp = tmp.to(unknown_map.dtype)

                # update seg_map, unknown_map_indicator, ratio, and unknown_map
                seg_map = seg_map * (1 - tmp) + tmp * unknow_class.unsqueeze(-1).unsqueeze(-1)

                unknown_map = torch.clip(unknown_map + tmp, 0, 1)
    ...
    def forward(
        ...
        # seg_mask = self.score2seg(batch['seg_score'], device=self.device)

        emb = data['embedding']

        # seg_mask = self.seg_model.getTranSizeResult(batch['image'], mask=None, size=self.content_codec.token_shape)
        # seg_mask = self.seg_model(batch['image'], mask=None, size=self.content_codec.token_shape)
        seg_mask = batch['segmentation_map']
        seg_mask = self.seg2unknown(seg_mask, batch['mask'])
        # assert seg_mask.max().item() < self.semantic_embedding.num_embeddings

        # self.sketch_model.eval()
        # batch['sketch'] = self.sketch_model(batch['image'][:,[2,1,0],:,:])
        ...
        p = np.random.uniform(low=0, high=1)
        if p < self.prob_sample['seg_prob']:
            # only seg
            flag_seg = 1
            flag_sketch = 0
        elif p < self.prob_sample['seg_prob'] + self.prob_sample['sketch_prob']:
            # only sketch
            flag_seg = 0
            flag_sketch = 1
        elif p < 1 - self.prob_sample['no_guidance_prob']:
            # seg and sketch
            flag_seg = 1
            flag_sketch = 1
        else:
        ...
        seg_emb = flag_seg * seg_emb + (1 - flag_seg) * self.seg_embed_default(torch.zeros((seg_emb.shape[0], seg_emb.shape[1], seg_emb.shape[2])).int().to(self.device))
        sketch_emb = flag_sketch * sketch_emb + (1 - flag_sketch) * self.sketch_embed_default(torch.zeros((seg_emb.shape[0], seg_emb.shape[1], seg_emb.shape[2])).int().to(self.device))
